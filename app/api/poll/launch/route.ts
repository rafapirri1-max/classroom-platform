import { launchPollActivity } from '@/lib/activity-instances'
import { validatePollLaunchInput } from '@/lib/poll/launch-config'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { room_id, teacher_id, question, options } = body
    ...
    const validated = validatePollLaunchInput(
      typeof question === 'string' ? question : '',
      options.map((o: unknown) => (typeof o === 'string' ? o : ''))
    )
    if (!validated.ok) {
    ...
    const activityInstanceId = await launchPollActivity(supabase, room, validated.config)
